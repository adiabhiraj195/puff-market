...
export default function BuyModal({ nftId, tokenId, price, sellerId, isOpen, onClose, onSuccess }: BuyModalProps) {
  ...
  const balanceFloat = parseFloat(puffBalance.replace(/,/g, '')) || 0;
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      console.log("[BuyModal] Step 1: Approving PUFF token spending...");
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-gray-800 bg-[#0c0d10] p-8 shadow-2xl text-white">
        ...
        <div className="bg-[#16171b] border border-gray-800 rounded-xl p-4 mb-6 flex flex-col gap-3">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-bold text-white text-lg">{Number(price).toLocaleString()} PUFF</span>
          </div>
            ...
            {isInsufficientBalance ? (
              <div className="flex flex-col gap-4">
                <div className="p-3 rounded-lg bg-yellow-950/30 border border-yellow-500/30 text-yellow-400 text-xs text-center">
                  You do not have enough PUFF tokens to buy this NFT.
                </div>
                <button
              ...
              >
                Buy for {Number(price).toLocaleString()} PUFF
              </button>
            )}
          </div>
        ) : (
            ...
            <div className={`p-4 rounded-xl border transition-all duration-300 ${step === 1 ? 'border-blue-500 bg-blue-950/10' : 'border-gray-800 bg-[#16171b]/20 opacity-60'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-lg flex items-center gap-2">
                  ...
                  Step 1: Approve PUFF Spending
                </span>
                <span className="text-xs font-medium uppercase text-blue-400">1 of 2</span>
              </div>
              <p className="text-gray-400 text-xs mb-3">Grant the marketplace contract permission to spend your PUFF.</p>
              {step1State !== 'idle' && (
